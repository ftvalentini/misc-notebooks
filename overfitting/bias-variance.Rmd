...
-	Variance $Var(\\hat{f}(X))$ : it represents how much the predicted values change when we fit the model using a different set of examples. If the predictions of a model are too dependent on the training set, then it is said it suffers from high **variance**. This means that *ceteris paribus* it will tend to perform poorly on other sets of examples i.e. it will have a low $MSE$. At the same time, the predictive performance on the training set will tend to be high i.e. it will have a high $MSE_{train}$. 
