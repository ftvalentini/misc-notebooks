...
donde cada estimador $h_m$ de aprende un *mapping* de ${\mathbf{x}}$ hacia $\mathbf{r}^{(m-1)}$. La magnitud de las predicciones de $h_m$ en cada iteración dependerá entonces de la magnitud de los gradientes $\mathbf{r}^{(m-1)}$ durante el entrenamiento, que decrecen tendencialmente a medida que crece $m$. De hecho, los gradientes negativos pueden entenderse como **residuos generalizados** entre $y$ y $\hat{y}$ que aplican a cualquier función de pérdida.  
